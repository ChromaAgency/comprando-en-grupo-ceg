...
class MagentoProductProduct(models.Model):
    ...
    magento_website_ids = fields.Many2many('magento.website', 'magento_product_product_website_ids_rel',
    ...
    short_description = fields.Html(string='Product Short Description',
    ...
    def _prepare_product_values(self, item):
        values = {
            'name': item.get('name') if item.get('name') else item.get('sku'),
            'default_code': item.get('sku'),
            # 'type': 'product' if not self._context.get('product_type_service') else 'service',
            'type': 'consu' if not self._context.get('product_type_service') else 'service',
            'is_storable': True if not self._context.get('product_type_service') else False,
    ...
    def import_configurable_product(self, line, item):
        ...
        if m_template:
            template = m_template.odoo_product_template_id
        if not template:
    ...
    def verify_configuration(self, line, item):
        ...
        if not instance.auto_create_product:
            ...
            if 'is_order' in (self.env.context.keys()):
                log_line.create_common_log_line_ept(message=message, module='magento_ept', order_ref=item.get('increment_id', ''),
                                                    default_code=item.get('sku'),
    ...
    def __verify_product_response(self, response, ids, line):
        ...
        for id in ids:
            ...
            if id not in response_ids:
                ...
                if key == 'order_ref':
                    log_line.create_common_log_line_ept(message=message, module='magento_ept', order_ref=line.magento_id,
                                                        magento_order_data_queue_line_id=line.id, model_name=self._name,
                                                        magento_instance_id=line.instance_id.id)
                else:
    ...
    def search_service_product(self, item, line):
        ...
        if not o_product:
            o_product = self.with_context(product_type_service=True)._search_odoo_product(line, item)
        if o_product:
            self.update_custom_option(item)
            ...
            message = _(f"Product SKU: {item.get('sku')} and Product Type: {item.get('type_id')} "
                        ...
                        f"in layer.")
            log_line.create_common_log_line_ept(message=message, module='magento_ept', default_code=item.get('sku', ''),
                                                magento_import_product_queue_line_id=line.id, model_name=self._name)
            return False
        return True
    ...
    def import_product_inventory(self, instance, auto_apply):
        ...
        if location:
            ...
            if consumable:
                log_line = self.create_consumable_products_log(consumable, log_line, instance)
        else:
    ...
    def prepare_import_stock_dict(self, response, instance):
        ...
        for item in items:
            ...
            if m_product:
                ...
                if qty > 0 and m_product.odoo_product_id.is_storable:
                    product_qty.update({m_product.odoo_product_id.id: qty})
                # elif m_product.odoo_product_id.type != 'product':
                elif not m_product.odoo_product_id.is_storable:
    ...
    @staticmethod
    def create_export_product_process_log(consumable, log_line, instance):
        """
        ...
        if consumable:
            ...
            product type is not 'Storable.'\n {str(list(set(consumable)))} 
            """
            log_line.create_common_log_line_ept(message=message, operation_type='export', module='magento_ept',
                                                model_name='magento.product.product', magento_instance_id=instance.id)

    @staticmethod
    def create_consumable_products_log(consumable_products, log_line, instance):
        """
        ...
        if consumable_products:
            ...
            log_line = log_line.create_common_log_line_ept(message=message, magento_instance_id=instance.id,
                                                           module='magento_ept', operation_type='import',
                                                           model_name='stock.quant')
        return log_line

    def exp_prd_stock_in_batches(self, stock_data, instance, api_url, data_key, method, log_line):
        """
    ...
    @staticmethod
    def call_export_product_stock_api(instance, api_url, data, log_line, method, line):
        """
        ...
        :param log_line: Common log_line object
        ...
        is_stock_exported = True
        if responses:
            for response in responses:
                if isinstance(response, dict) and response.get('code') != '200':
                    is_stock_exported = False
                    log_line.create_common_log_line_ept(message=response.get('message'), module='magento_ept',
                                                        magento_export_stock_queue_line_id=line.id,
                                                        magento_instance_id=instance.id)
        return is_stock_exported
    ...
    def export_magento_stock(self, line, api_url, log_line):
        instance = line.instance_id
        data = json.loads(line.data)
        is_stock_exported = True
        if data:
            if instance.is_multi_warehouse_in_magento:
                is_stock_exported = self.call_export_product_stock_api(instance, api_url, data, log_line, 'POST', line)
            else:
                is_stock_exported = self.call_export_product_stock_api(instance, api_url, data, log_line, 'PUT', line)
        return is_stock_exported
