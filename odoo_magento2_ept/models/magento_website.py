    ...
    def get_shipped_orders(self, record):
        ...
        shipped_query = """
            ...
            WHERE 
                ...
                sp.state = 'done' AND
                so.magento_instance_id = %s AND
                so.magento_website_id = %s AND
                sl.usage='customer'
        """
        ...
        def website_vise_shipped_order_of_current_week(shipped_query):
            ...
            self._cr.execute(qry, (record.magento_instance_id.id, record.id))
            return self._cr.dictfetchall()
        ...
        def website_vise_shipped_order_of_all_time(shipped_query):
            self._cr.execute(shipped_query, (record.magento_instance_id.id, record.id))
            return self._cr.dictfetchall()
