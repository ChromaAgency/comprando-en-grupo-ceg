...
class ReportSaleOperationsTradeUnity(Controller):
    ...
    def _add_row_data_to_csv(self, file_content, row): 
        ...
        product_component = row.product_id if row.move_id.bom_line_id else request.env['product.product']
        ...
        data = [sale_order.name, customer_commercial.id, customer.display_name, customer_phone,customer_mobile, product.ceg_code or '', product_component.ceg_code or '',
                   product.name, UNIDAD, move_qty*box_qty, move_qty,move_qty*product.volume, REFERENCIA,delivery_mode,shipping_address.contact_address_complete, shipping_address.street_name, shipping_address.street_number,
    ...
    def _add_header_to_csv(self, file_content):
        headers = ['Pedido de ventas', 'Cuenta de cliente', 'Nombre', 'Teléfono del cliente','Teléfono móvil del cliente', 'Código de artículo',	'Código de artículo componente',
               'Nombre del producto','Unidad','Cantidad','Bultos','Volumen','Referencia','Modo de entrega','Dirección', 'Nombre de calle',  'Número de calle', 'Calle 2', 'Ciudad', 'Código Postal', 'Estado', 'País',
